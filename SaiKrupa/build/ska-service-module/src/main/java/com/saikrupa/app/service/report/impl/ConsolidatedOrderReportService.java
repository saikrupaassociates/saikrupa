...
public class ConsolidatedOrderReportService extends AbstractReportService {
	
	private List<OrderData> orders;

	public ConsolidatedOrderReportService(List<OrderData> orders) {
		setOrders(orders);
	}
	...
	public void createPageTableContent(Document document) throws DocumentException {
		...
		ReportTableHeaderCell header_contact = new ReportTableHeaderCell("Contact");
		...
		dataTable.addCell(header_contact);
		...
		for(OrderData data : getOrders()) {
			...
			ReportTableDataCell data_contact = new ReportTableDataCell(data.getCustomer().getPrimaryContact().toString(), CellValueType.TEXT);
			...
			dataTable.addCell(data_contact);
	...
	public void addMetaData(Document document) {
		document.addTitle("Consolidated Order Report - All Orders");
		
	}

	@Override
	public String getPageTitle() {
		return "Order Report - Consolidated";
	}
